    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-4 flex items-center justify-between">
                <p class="text-sm text-gray-600 dark:text-gray-300">Browse products and add them to your cart.</p>
                <a href="{{ route('cart.index') }}" class="relative inline-flex items-center gap-2 rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-indigo-500">
                ...
                @forelse ($products as $product)
                    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                        @if ($product->image_path)
                            <img src="{{ asset('storage/'.$product->image_path) }}" alt="{{ $product->name }}" class="h-48 w-full object-cover">
                        @else
                            <div class="h-48 w-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center text-sm text-gray-500">
                                No image
                            </div>
                        @endif
                        ...
                        <div class="p-4 text-gray-900 dark:text-gray-100">
                            ...
                            <p class="mt-2 text-lg font-bold">P{{ number_format($product->price, 2) }}</p>

                            <form method="POST" action="{{ route('cart.add', $product) }}" class="mt-4 flex items-center gap-2">
                                @csrf
                                <input
                                    type="number"
                                    name="quantity"
